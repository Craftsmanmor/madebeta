...
class UserModel
{
    ...
    public static function getPublicProfilesOfAllUsers()
    {
        ...
        $sql = "SELECT * FROM users s JOIN person sp ON sp.user_email = s.user_email WHERE user_id = :user_id LIMIT 1";
        ...
        foreach ($query->fetchAll() as $user) {
            ...
            $all_users_profiles[$user->user_id]->user_email = $user->firstname;
     ...
     * @return bool
     */
    public static function doesIDAlreadyExist($identity)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("SELECT identity FROM person WHERE identity = :identity LIMIT 1");
        $query->execute(array(':identity' => $identity));
        if ($query->rowCount() == 0) {
            return false;
        }
        return true;
    ...
    public static function doesTAXAlreadyExist($tax_number)
    {
        ...
        $query = $database->prepare("SELECT tax_number FROM person WHERE tax_number = :tax_number LIMIT 1");
